...
import { filterRole, filterStatus, olashoreUserData } from "./OlashoreUserData";
import { bulkActionOptions, findUpper } from "../../utils/Utils";
import { Link } from "react-router-dom";
import { OlashoreUserContext } from "../../context/OlaUserContext";
import EditModal from "./EditModal";
import AddModal from "./AddModal";

const UserList = () => {
  const { contextData } = useContext(OlashoreUserContext);
  ...
  useEffect(() => {
    let newData;
    newData = olashoreUserData.map((item) => {
      item.checked = false;
      return item;
    });
    setData([...newData]);
  ...
  useEffect(() => {
    if (onSearchText !== "") {
      const filteredObject = olashoreUserData.filter((item) => {
        return (
